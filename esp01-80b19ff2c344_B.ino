//2022-6-13
//change GOIO0 to button input mode
...
int ESP01S_button =0; 
int ESP01S_LED = 2;//模块内置led
//int ESP01S_button = 0;
int relay_state=0;
unsigned long previousMillis = 0; //预设的记录毫秒数
int interval =2000; //间隔的毫秒数
 ...
 void heartbeat()  //心跳包
{
    ...
    } else if(digitalRead(ESP01S_LED) == HIGH){
...
void loop() {
    Blinker.run();//Blinker.run()语句负责处理Blinker收到的数据，每次运行都会将设备收到的数据进行一次解析。//在使用WiFi接入时，该语句也负责保持网络连接
     ...
     if(digitalRead(ESP01S_button) == LOW){
        delay(200);//wait key jitter
          if(digitalRead(ESP01S_button) == LOW){
            if(relay_state == 30){ 
                //Button1.print("on");
                digitalWrite(ESP01S_LED,LOW);
    ...
    unsigned long currentMillis = millis(); //取系统运行毫秒数
